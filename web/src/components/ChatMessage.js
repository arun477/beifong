...
const PonyoChatIcon = ({
   ...
   // Add a periodic "heartbeat" effect
   const [heartbeat, setHeartbeat] = useState(false);

   // Add rays of light that rotate
   const [rayAngle, setRayAngle] = useState(0);

   // Create a periodic heartbeat effect when not loading
   useEffect(() => {
      if (isLoading) return;
   ...
   // Create rotating light rays effect
   useEffect(() => {
      if (isLoading) return;
   ...
   return (
      ...
      >
         ...
         {/* Improved background with subtle radial gradient */}
         ...
         >
            ...
            {/* Ponyo image with improved positioning */}
            <div
  ...
  return (
    <div className="my-2">
      ...
      <div className="bg-gray-900/60 backdrop-blur-md rounded-lg p-3 flex items-center relative overflow-hidden border border-white/10 shadow-lg">
        {/* Glassmorphic highlight effects */}
        ...
        <div className="flex-1 flex flex-col z-10">
          ...
          {/* Elegant loading bar with glassmorphic effect */}
      ...
      <style jsx>{`
        .bg-grid-pattern {
          background-image: radial-gradient(circle, rgba(16, 185, 129, 0.2) 1px, transparent 1px);
